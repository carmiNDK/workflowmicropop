	...
	<div class="content">
			...
			<div class="content-nav">
				<ul class="nav nav-tabs" id="myTab" role="tablist">
					...
					<li id="InfoFinanceClientGL" class="nav-item" role="presentation"><a
						href="InfoFinanceClientGL.php" class="nav-link active"
						type="button" role="tab" aria-selected="false">Informations-Financieres</a></li>
		...
		<div class="ui blue segment table-responsive"
				...
				<div class="row">
					...
					<div class="four wide column"
						...
						<button id="btnAdd" type="button" class="ui primary button"
							...
							<c:if test="${sessionScope.etatGroupGL != 'Analyse'}">
                                                                        disabled
                                                        </c:if>>
							Ajouter</button>
					</div>
			...
			<table class="ui celled table" id="tab_AB_Stock">
				<thead>
					<tr>
						<th><label for="">N°</label></th>
						<th><label for="">NomsClientMembre</label></th>
						<th><label for="">TotalAPayer($)</label></th>
						<th><label for="">MontantPretAcc($)</label></th>
						<th><label for="">EpargneBrute($)</label></th>
						<th><label for="">Epargne($)</label></th>
						<th><label for="">Remboursement($)</label></th>
						<th><label for="">TotalRemboursement($)</label></th>
						<th><label for="">Commentaire</label></th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${ListeInformationFinancireClientGL}" var="SPP">
						<tr>
							...
							<td><c:if
									...
									<a class="ui yellow mini button"
										href="GetInformationFinancireClientGL.php?numinfoFinancegl=${SPP.idinfofinanciereglglgl}"><i
										class="edit icon"></i></a>
								</c:if></td>
						</tr>
					</c:forEach>
	...
	<div class="ui modal" id="exampleModal">
		...
		<div class="scrolling content">
			<h2 class="tw-text-lg tw-font-semibold tw-mb-2">Modification
				Information Financiere Client GL</h2>
			<hr>
			<form class="ui form" action="UpdateInfoFinanciereClientGL.php"
				method="post">
				<input type="hidden" id="hid_id_client"
					value="${IdInfoFinanciereGL}" name="IdInfoFinanciereGL">
				<div class="fields">
					<div class="eight wide column field">
						<label>NomsMembreGroupe :</label> <select
							name="NomsMembreGroupeGL" id="NomsMembreGroupeGL"
							class="ui dropdown" required style="width: 100%;">
					...
					<div class="eight wide column field">
						<label>MontantEpargne [Reunion] ($) :</label> <input type="number"
							id="MontantEpargneReunionGL" name="MontantEpargneReunionGL"
							required value="${MontantEpargneReunionGL}" min="0" step="0.01" />
					</div>
				</div>

				<div class="field">
					<label>Commentaire :</label>
					<textarea name="commentaireInfoFinanciereCliGL"
						id="commentaireInfoFinanciereCliGL" rows="6" required>${commentaireInfoFinanciereCliGL}</textarea>
				</div>

				<div class="actions"
					style="background: #f9f9f9; padding: 15px; border-top: 1px solid #ddd; margin-top: 20px;">
					<a href="InfoFinanceClientGL.php" class="ui primary button">
						Annuler </a>
					<button type="button"
