...
<title>KwikLoan : Paramètrage Groupe</title>
</head>
...
<body>
			...
			<div class="content-nav">
				<ul class="nav nav-tabs" id="myTab" role="tablist">
					...
					<li id="menuFamille" class="nav-item" role="presentation"><a
						href="ParametrageGLPage.php" class="nav-link active" type="button"
		...
		<div class="page-content">
			...
			<div class="ui blue segment">
				<h2 class="tw-text-lg tw-font-semibold tw-mb-2">Configuration
					Produit Crédit GL</h2>
				<hr>
				<form class="ui form" action="ParametrageGLPage.php" method="post">
					<input type="hidden" value="${numPGL}" name="numPGL">
					<div class="two fields">
						<div class="field">
							<label>CoefficientDégressif </label> <input type="number"
								id="coefficientDegressif" name="coefficientDegressif"
								required="required" value="${coefficientDegressif}" min="0"
								step="0.0001"></input>
						</div>

						<div class="field">
							<label>MntFraisTenuCompte </label> <input type="number"
								class="form-control" id="MntFraisTenuCompte"
								name="MntFraisTenuCompte" required="required"
								value="${MntFraisTenuCompte}" min="0" step="0.01"></input>
						</div>
					</div>

					<div class="two fields">
						<div class="field">
							<label>Nombre Max des membres </label> <input type="number"
								class="form-control" id="NbrMaxMembreGL" name="NbrMaxMembreGL"
								required="required" value="${NbrMaxMembreGL}" min="0"
								step="0.01"></input>
						</div>
						<div class="field">
							<label>Nombre d'échéances </label> <input type="number"
								class="form-control" id="NbrEcheanceGL" name="NbrEcheanceGL"
								required="required" value="${NbrEcheanceGL}" min="0" step="0.01"></input>
						</div>
					</div>

					<div class="three fields">
						<div class="field">
							<label>FraisSMSGL </label> <input type="number"
								class="form-control" id="FraisSMSGL" name="FraisSMSGL"
								required="required" value="${FraisSMSGL}" min="0" step="0.01"></input>
						</div>
						<div class="field">
							<label>FraisCarnetGL</label><input type="number"
								class="form-control" id="FraisCarnetGL" name="FraisCarnetGL"
								required="required" value="${FraisCarnetGL}" min="0" step="0.01"></input>
						</div>
						<div class="field">
							<label>TauxCautionGL </label> <input type="number"
								class="form-control" id="TauxCautionGL" name="TauxCautionGL"
								required="required" value="${TauxCautionGL}" min="0" step="0.01"></input>
						</div>
					</div>

					<div class="three fields">
						<div class="field">
							<label>FraisDossierGL </label><input type="number"
								class="form-control" id="FraisDossierGL" name="FraisDossierGL"
								required="required" value="${FraisDossierGL}" min="0"
								step="0.01"></input>
						</div>
					</div>
				...
				<table class="ui celled table" id="tab_AB_Stock">
					<thead>
						<tr>
							...
							<th><label for="quantite">CoefDegressif</label></th>
							<th><label for="produit">mntFraisTenuCompte</label></th>
							<th><label for="id">nbrMaxMembreGL</label></th>
							<th><label for="quantite">NbrEcheance</label></th>
							<th><label for="produit">FraisSMSGL</label></th>
							<th><label for="id">FraisCarnet</label></th>
							<th><label for="quantite">TauxCaution</label></th>
							<th><label for="produit">FraisDossierGL</label></th>
							<th><label for="action">Action</label></th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${InformationParametrageGL}" var="SPPM">
							<tr>
								<td>${SPPM.idparamgl}</td>
								<td>${SPPM.coefdegressifgl}</td>
								<td>${SPPM.mntfraistenucomptegl}</td>
								<td>${SPPM.nbrmaxmembregl}</td>
								<td>${SPPM.nbrecheancegl}</td>
								<td>${SPPM.fraissmsgl}</td>
								<td>${SPPM.fraiscarnetgl}</td>
								<td>${SPPM.tauxcautiongl}</td>
								<td>${SPPM.fraisdossiergl}</td>
								<td><a class="ui mini yellow icon button btn-edit"
									href="ReturneInformationParaGL.php?numPGL=${SPPM.idparamgl}"><i
	...
	<c:if test="${nbrUserSavePMGL > 0}">
		<script>
			Swal.fire({
				title : 'Succès !',
				text : 'Le paramétrage GL est enregistrer avec succés !!!',
				icon : 'success',
				confirmButtonText : 'OK'
			});
		</script>
	</c:if>
	<c:if test="${nbrUserUpdatePMGL > 0}">
		<script>
			Swal.fire({
				title : 'Succès !',
				text : 'Le paramétrage GL est modifier avec succés !!!',
				icon : 'success',
				confirmButtonText : 'OK'
			});
		</script>
