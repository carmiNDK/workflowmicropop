...
<body>
	...
	<div class="ui modal" id="modalProduction">
		...
		<div class="content">
			...
			<form class="ui form" action="InfoBussness3CoutProductionProd.php"
				...
				<div class="field">
					<label>PrixVenteParUnité($) :</label> <input type="number"
	...
	<div class="ui modal" id="exampleModal2b20020011025">
		...
		<div class="content">
			...
			<form class="ui form" action="InfoBussness3MatiereProd.php"
				...
				<div class="field">
					<label>PrixAchatMatiere($) :</label> <input type="number"
	...
	<div class="ui modal" id="exampleModal2b200200">
		<div class="content">
			<h2 class="tw-text-lg tw-font-semibold tw-mb-2"
				style="margin-top: 15px;">MARGE BRUTE MOYENNE PONDEREE (MBMP)</h2>
			<hr>
			<form class="ui form" action="InfoBussness3MBMP.php" method="post">

				<div class="fields">
					<div class="eight wide column field">
						<label>LibelleProduit :</label> <input type="text"
							name="LibelleProduitMPMP" required>
					</div>
					<div class="eight wide column  field">
						<label>CoutParUnite ($) :</label> <input type="number"
							name="CoutParUniteMPMP" required min="0" step="0.01">
					</div>
				...
				<div class="fields">
					...
					<div class="six wide column field">
						<label>PrixVente ($) :</label> <input type="number"
							name="PrixVenteMPMP" required min="0" step="0.01">
					</div>
					<div class="six wide column field">
						<label>MargeProduit($) :</label> <input type="number"
							name="MargeProduitMPMP" required min="0" step="0.01">
					</div>
				...
				<div class="field">
					<label>CoutMensuel ($) :</label> <input type="number"
						name="CoutMensuelMPMP" required min="0" step="0.01">
				</div>
				<div class="field">
					<label>VenteMensuel ($) :</label> <input type="number"
						name="VenteMensuelMPMP" required min="0" step="0.01">
				</div>
				<button type="submit" class="ui primary button">Enregistrer</button>
			</form>
		</div>
	...
	<div class="ui modal" id="exampleModal1000252525">
		<div class="content">
			<div class="ui header"
				style="margin-bottom: 15px; font-size: 14px; margin-top: 10px; margin-left: 70px;">Information
				Matière / Production</div>
				...
				<div class="ui grid">
					<div class="sixteen wide column">
						<table class="ui celled table">
							...
							<tbody>
								<c:forEach items="${ListeMatiereProduction}" var="item">
									<tr>
										<td>${item.idmaterielprod}</td>
										<td>${item.libellematiereprod}</td>
										<td>${item.prixachatmatierepro}</td>
										<td>${item.couttotalprod}</td>
										<td><a class="ui mini red icon button btn-delete"
											data-id="" onclick="$('.ui.modifier-modal').modal('show');"
											data-url="SupInformationMP.php?numMP=${item.idmaterielprod}">
					...
					<div class="twelve wide column">
						<strong>Coût Total Général :</strong>
					</div>
					<div class="four wide column">
						<div class="ui input fluid">
							<input type="text" readonly value="${coutTotalG}">
						</div>
					</div>

					<!-- Coût par unité -->
					<div class="twelve wide column">
						<strong>Coût de production (par unité) :</strong>
					</div>
					<div class="four wide column">
						<div class="ui input fluid">
							<input type="text" readonly value="${coutTotalParUnite}">
						</div>
					</div>
		...
		<div class="actions">
			<div class="ui red deny button">Fermer</div>
		</div>
	</div>
	...
	<script>
	const links = document.querySelectorAll('.addMatProd');

	  links.forEach(link => {
	    link.addEventListener('click', function(e) {
	      e.preventDefault(); // empêche le comportement par défaut du lien
	      const id = link.getAttribute('data-id1'); // récupère l'id
	      document.getElementById('IdProduitProductionProd').value = id; // injecte dans le hidden
	      $('#exampleModal2b20020011025').modal('show'); // ouvre le modal Fomantic UI
	    });
	  });
		$('#btnAddCout').click(function() {
			$('#modalProduction').modal('show');
		});
	

		$('#addProduit').click(function() {
			$('#exampleModal2b200200').modal('show');
		});
	</script>
	...
	<c:if test="${showModalDetails == true}">
		<script>
			$(document).ready(function() {
				$('#exampleModal1000252525').modal('show');
			});
		</script>
	</c:if>
