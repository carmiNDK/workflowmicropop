...
<body>
			...
			<div class="content-nav">
				<ul class="nav nav-tabs" id="myTab" role="tablist">
					...
					<li id="menuBussness1" class="nav-item" role="presentation"><a
						href="ParametrageBranchePage.php" class="nav-link active"
		...
		<div class="page-content">
			...
			<div class="ui blue segment">
				<h2 class="tw-text-lg tw-font-semibold tw-mb-2">Configuration
					agence</h2>
				<hr>
				<form class="ui form" action="ParametrageEnrBranchePage.php"
					method="post">
					<input type="hidden" id="idBrancheMcpop" name="idBrancheMcpop"
						value="${idBrancheMcpop}">
					<div class="two fields">
						<div class="field">
							<label>LibelleBranche :</label> <input type="text"
								class="form-control" id="LibelleBrancheMCPOP"
								name="LibelleBrancheMCPOP" required="required"
								value="${LibelleBrancheMCPOP}"></input>
						</div>

						<div class="field">
							<label>AdresseBranche :</label> <input type="text"
								class="form-control" id="AdresseBrancheMCPOP"
								name="AdresseBrancheMCPOP" required="required"
								value="${AdresseBrancheMCPOP}"></input>
						</div>
					</div>

					<div class="two fields">
						<div class="field">
							<label>TelephoneBranche :</label> <input type="text"
								class="form-control" id="TelephoneBrancheMCPOP"
								name="TelephoneBrancheMCPOP" required="required"
								value="${TelephoneBrancheMCPOP}"></input>
						</div>

						<div class="field">
							<label>ProvinceBranche :</label> <select
								id="ProvinceBrancheMCPOP" class="ui dropdown"
								name="ProvinceBrancheMCPOP" required="required">
								<option selected>${ProvinceBrancheMCPOP}</option>
					...
					<div class="two fields">
						<div class="field">
							<label>ResponsableBranche :</label> <input type="text"
								class="form-control" id="ResponsableBrancheMCPOP"
								name="ResponsableBrancheMCPOP" required="required"
								value="${ResponsableBrancheMCPOP}"></input>
						</div>
				...
				<table class="ui celled table" id="tab_AB_Stock">
					<thead>
						<tr>
							...
							<th><label for="quantite">LibelleBranche</label></th>
							<th><label for="produit">AdresseBranche</label></th>
							<th><label for="prix_dachat">TelephoneBranche</label></th>
							<th><label for="prix_devente">ProvinceBrnche</label></th>
							<th><label for="marge_produit">ResponsableBranhe</label></th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${ListeListeBrancheMCPOPAll}" var="SPP">
							<tr>
								<td>${SPP.idbranchemcpop2025}</td>
								<td>${SPP.libellebranchemcpop2025}</td>
								<td>${SPP.adressebranchemcpop2025}</td>
								<td>${SPP.telephonemcpop2025}</td>
								<td>${SPP.provincemcpop2025}</td>
								<td>${SPP.responsableagencemcpop2025}</td>
								<td><a class="ui mini yellow icon button btn-edit"
									href="ReturneInformationBrancheMcpop.php?numB=${SPP.idbranchemcpop2025}"><i
	...
	<c:if test="${nbrUserSaveBrancheMCPOP > 0}">
		<script>
			Swal.fire({
				title : 'Succès !',
				text : 'La branche est enregistrer avec succés !!!',
				icon : 'success',
				confirmButtonText : 'OK'
			});
		</script>
	</c:if>
	<c:if test="${nbrUserUpdateBrancheMCPOP > 0}">
		<script>
			Swal.fire({
				title : 'Succès !',
				text : 'La branche est modifier avec succés !!!',
				icon : 'success',
				confirmButtonText : 'OK'
			});
		</script>
