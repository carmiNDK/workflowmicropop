	...
	<div class="content">
			...
			<div class="content-nav">
				<ul class="nav nav-tabs" id="myTab" role="tablist">
					...
					<li id="MembreQuitte" class="nav-item" role="presentation"><a
						href="MembreQuitte.php" class="nav-link active" type="button"
						role="tab" aria-selected="false">Membre/Quitte</a></li>
					...
					<div class="row">
						...
						<div class="four wide column"
							...
							<button id="btnAdd" type="button" class="ui primary button"
								...
								<c:if test="${(sessionScope.etatGroupGL eq 'Comite') or (sessionScope.etatGroupGL eq 'Verification')or (sessionScope.etatGroupGL eq 'Postage') or (sessionScope.etatGroupGL eq 'Terminer')or (sessionScope.etatGroupGL eq 'Rejeter')}">
                                                                        disabled
                                                        </c:if>>
								Ajouter</button>
						</div>
				...
				<table class="ui celled table" id="tab_AB_Stock">
					<thead>
						<tr>
							...
							<th><label for="id">NumMembre</label></th>
							<th><label for="quantite">NomsClientG</label></th>
							<th><label for="prix_dachat">DateQuitte</label></th>
							<th><label for="produit">CycleCrédit</label></th>
							<th><label for="marge_produit">Raison</label></th>
							<th><label for="valeur_stock">MontantEpargne</label></th>
							<th><label for="valeur_stock">Statut</label></th>
							<th><label for="valeur_stock">CommentaireStatut</label></th>
							<th><label for="valeur_stock"></label></th>
						</tr>
					</thead>
					<tbody>
						<c:forEach
							items="${ListetListeInformationClientQuitteGroupeCycle}"
							var="SPPQ">
							<tr>
								<td>${SPPQ.idquitte}</td>
								...
								<td>${SPPQ.statutquitte}</td>
								<td>${SPPQ.commquitte}</td>
								<td><c:if test="${sessionScope.etatGroupGL eq 'Analyse'}">
										<a class="ui mini green button"
											href="ReintegrationMembreGL.php?numclient=${SPPQ.numclientquitte}&&iddd=${SPPQ.idquitte}">Réintégration</a>
									</c:if></td>
								<td><a title="Voir la photo du Membre GL"
									href="javascript:void(0)"
									onclick="openClientModal(${SPPQ.numclientquitte}, '${SPPQ.nomsclientglquite}')">
										<i class="eye icon"></i>
								</a></td>
							</tr>
						</c:forEach>
		...
		<div class="ui modal" id="exampleModal">
			<div class="scrolling content">
				<h2 class="tw-text-lg tw-font-semibold tw-mb-2">Information
					Client GL / Quitte</h2>
				<hr>
				<form class="ui form" action="EnregistrementClientGLQuitte.php"
					method="post">
					...
					<div class="fields">
						...
						<div class="eight wide column field">
							<label>DateQuitte :</label> <input type="date"
								id="DateQuitteClientGL" name="DateQuitteClientGL" required>
						</div>

					</div>

					<div class="field">
						<label>Epargne-Retour :</label> <input type="number"
							name="EpargneRetour" id="EpargneRetour" required min="0"
							step="0.01"></input>
					</div>

					<div class="field">
						<label>Raison :</label>
						<textarea name="RaisonSocialQuitte" id="RaisonSocialQuitte"
		...
		<c:if test="${integ > 0}">
			<script>
				Swal
						.fire({
							title : 'Succès !',
							text : 'ce membre est réintégrer avec succés dans le groupe!!!',
							icon : 'success',
		...
		function openClientModal(idclient, nomsCompletCliGL) {
		    document.getElementById('clientModalImg').src =
		        'ReturnPhotosClient3.php?idclient=' + idclient +
		        '&nomsCompletCliGL=' + encodeURIComponent(nomsCompletCliGL);
